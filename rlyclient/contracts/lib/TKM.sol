...
library TKM {
    ...
    function headerPropertyHash(uint32 chainid, uint64 height, uint8 index) internal pure returns(bytes32) {
        bytes memory posBuffer = abi.encodePacked(bytes4(chainid), bytes8(height), bytes1(index));
        return keccak256(posBuffer);
    }

    function isHeaderPropertyHash(MPT.MerkleProof[] memory proofs, uint32 chainid, uint64 height, uint8 index) internal pure returns(bool) {
        bytes32 posHash = MPT.HashAtReverseIndex(proofs, 6);
        bytes memory posBuffer = abi.encodePacked(bytes4(chainid), bytes8(height), bytes1(index));
        bytes32 ph = keccak256(posBuffer);
        return posHash == ph;
    }
    ...
    function bytes32ToString(bytes32 value) internal pure returns (string memory) {
        ...
        for (uint i=0; i<32; i++) {
            uint8 digit = uint8(value[i] & 0xF0) >> 4;
            buffer[i*2] = char(digit);
            digit = uint8(value[i]&0x0F);
            buffer[i*2+1] = char(digit);
        }
    ...
    function hashReceiptV2(ReceiptData memory receiptData) internal pure returns (bytes memory logBytes, bytes32 rcptHash) {
        require(receiptData.rcpt.Version>=2, "receipt version should be v2");
        ...
        bytes memory rcptBytes = encodeReceiptV2(receiptData.rcpt, logRoot);
