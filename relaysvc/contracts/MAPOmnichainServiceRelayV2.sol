...
contract MAPOmnichainServiceRelayV2 is ReentrancyGuard, Initializable, Pausable, IMOSV2, UUPSUpgradeable {
    ...
    // record distribute rate
    mapping(uint => Rate) public distributeRate;
    // record out event order
    mapping(bytes32 => bool) public orderList;
    // record mos address of from chain
    mapping(uint256 => bytes) public mosContracts;
    // record chain VM type
    ...
    function initialize(address _managerAddress) public initializer
    ...
    function tokenWithdraw(address _token, address payable _receiver, uint256 _amount) external onlyOwner checkAddress(_receiver) {
        ...
        if (chainTypes[_chainId] == chainType.NEAR) {
            ...
            for (uint256 i = 0; i < logs.length; i++) {
                ...
                if (topic == EvmDecoder.MAP_TRANSFEROUT_TOPIC) {
                    ...
                    //require(Utils.checkBytes(mosContract, mosContracts[_chainId]), "invalid relayservice contract");
